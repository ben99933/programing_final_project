    ...
    for(n = 0.0; n < bs; n = n + 1.0){
      for(s = 0.0; s < ss; s = s + 1.0){
        ...
        if(settings->barBorder){
          ...
          char output[6];
          snprintf(output, 5, "%lf", yValue);

          wchar_t *output_w  = malloc (sizeof(wchar_t) * (strlen(output) + 1));
          mbstowcs(output_w, output, strlen(output) + 1);
          
          DrawText(canvas, Round(x) + 40, Round(y) - 22.5, output_w, strlen(output) + 1, GetBlack());
